	...
	<c:forEach var="ticket" items="${tickets}">
		<div class=enclose1>
			<form ticket="${pageContext.request.contextPath}/TicketList.java" method="post" onsubmit="saveData()">
				...
				<h5>
					日時: ${ticket.gameDate} ${ticket.startTime} <br /> 場所: ${ticket.stadiumName} <br /> 座席: ${ticket.seatType}
					${ticket.seatNumber} <br /> 価格: ${ticket.seatPrice}円
				</h5>
			</form>
			<br />
			<form action="${pageContext.request.contextPath}/TicketList" method="post">
				<!-- ticketIdが正しく設定されていることを確認 -->
				<input type="hidden" name="ticketId" value="${ticket.ticketId}" /> <input type="hidden" name="resale"
					value="resaleTicketSalesConfirmation" />
				<button type="submit" class="button1">リセール</button>
			</form>
			...
			<form action="${pageContext.request.contextPath}/TicketList" method="post">
				...
				<input type="hidden" name="ticketId" value="${ticket.ticketId}" /> <input type="hidden" name="resale"
					value="ticketDisplay" />
				<button type="submit" class="button1">QRコード</button>
			</form>
